...
The initial research and design conversation is complete, `PLAN.md` is approved, and implementation is underway. Slices 0–5 and 7 and Gates A–D are complete and merged; Slice 6 is active pending its simulation review. Gate C's 5x5 map/HUD grammar and Slice 4's ten validated authored topologies, deterministic alternate hidden exits, persistent frontier and dead-end discovery, exact map projection, two-way movement, and backtracking were merged on 2026-08-10 after owner playtesting through the real controller loop. A 2026-08-12 topology-quality follow-up replaced the nine structurally linear maps with explicit branches and loops and shifted most eligible exits deeper. Every map now has multiple junctions and at least one loop; hidden exits remain seven to ten shortest-path moves away and average at least 8.5 across the catalog. Final Slice 5 physical review was approved on 2026-08-11. The Wizard scroll flows and Rogue Steal/Unaware passed owner Board Lab review on 2026-08-12. Class-specific ordinary battle loot passed owner physical review on 2026-08-23: Warrior Iron Sword/Chain Mail, Rogue Shadow Knife/Night Cloak, and Wizard Ash Wand/Rune Robe all use the board-first Equip/Leave flow. The owner also physically approved replacing solid color roll tracks with four board-native period characters on 2026-08-23. The Room of Blades success, cache/equip, stat application, persistence, and resolved-room no-retrigger flows passed owner Board Lab review on 2026-08-26. The Ancient Library Wizard success, replacement-scroll, persistence, and resolved-room no-retrigger flows passed owner Board Lab review on 2026-08-26; the Skeleton Knight ambush branch remains automated-test verified rather than owner-playtested. The Chained Prisoner Question success, truthful clue, Leave persistence, and resolved-room no-retrigger flows passed owner Board Lab review on 2026-09-13. The Strange Hole Look success, truthful direction clue, board presentation, and pacing passed owner Board Lab review on 2026-09-13; its Reach/cache/injury branches remain automated-test verified. On 2026-09-13 the owner closed the private-alpha event roster at those five implemented events and approved selecting three per run, independently of class, with off-route and graph-spacing preferences; a complete Board Lab run then approved the resulting event density. The remaining three seeds moved to content beta. The first 3,000-run class-readiness probe reached the final exit approach in 99.8% of Warrior runs, 64.4% of Rogue runs, and 80.1% of Wizard runs with no command-limit stalls; treat the spread as Gate E evidence rather than silently locking final balance. Preserve the owner's ordinary board transition for every game frame; VestaQuest must not force Wave/Fast for roll results. Local API refinement and staged per-die reveals remain optional later work. Preserve the owner-confirmed direction below. Do **not** silently settle the remaining open balance, content, distribution, or technology decisions; resolve them at the decision gates in `PLAN.md` or bring them back to the owner.
...
- **Library:** the approved first implementation is `ANCIENT LIBRARY`, showing `THE SHELVES WHISPER` with `SEARCH / LEAVE`. Search rolls `D6 + POWER` versus `D6 + DANGER 4`; the Wizard rolls `2D6` keep-high and ties fail. Approximate Level 1 success odds are Wizard 75%, Warrior 58%, and Rogue 28%. Search is attempted once per run; Leave preserves the attempt.
- A successful Wizard search with room in the three-slot pouch finds one random Fireball, Lightning, or Stun scroll; duplicates are possible. Otherwise, success takes a Healing Draught if the general consumable slot is empty, or shows `ONLY DEAD WORDS REMAIN` when the hero is fully stocked. Failure shows `THE SHELVES AWAKEN` and creates a persistent Skeleton Knight ambush in the Library room. The ambusher uses normal initiative, retreat/re-entry, and battle-loot rules unless owner review changes that provisional consistency choice.
- The Ancient Library Wizard success path passed owner Board Lab review on 2026-08-26. The owner spent Fireball against a Ghoul, equipped the Ash Wand, successfully searched with the Wizard keep-high roll, received a replacement Lightning scroll (`LIGHTNING / STUN / LIGHTNING`), observed the correct Level 3 totals (`HP4 POW7 DEF2 SKL3 LCK4`), and confirmed that leaving and re-entering the resolved Library did not retrigger it. The failure-to-ambush branch remains covered by automated game and replay tests pending owner review.
- The Chained Prisoner is class-neutral. It presents `FREE / QUESTION / LEAVE`. Free rolls raw `2D6` keep-high versus raw `D6`, with ties succeeding (about 75%); failure deals 1 HP and can kill with cause `THE CHAINS`. Question rolls raw `D6` versus raw `D6`, with ties failing (about 42%); failure is harmless. A successful Free or Question reveals a transient truthful first direction on a real shortest path to the hidden exit. Either attempt resolves the event, while an initial Leave preserves it. No class receives different odds.
- The Chained Prisoner's Question success, truthful-direction clue, initial Leave preservation, and resolved-room no-retrigger flows passed owner Board Lab review on 2026-09-13. The Free injury/death branches remain automated-test verified rather than owner-playtested.
...
- **Room of traps:** the approved first implementation is `ROOM OF BLADES`, an optional trapped-cache challenge rather than a required route gate. The board shows `A CACHE WAITS BEYOND` with `CROSS / LEAVE`. Cross rolls `D6 + SKILL` versus `D6 + DANGER 3`; ties fail, giving approximate Level 1 success odds of Rogue 72%, Wizard 42%, and Warrior 28%. Success opens the same equal-eligible dust, Healing Draught, and missing-slot class-equipment cache used by the Solid Door. Ordinary failure deals 1 HP; losing by 3+ deals 2 HP. A raw player `1` against Danger `6` causes immediate death with `THE BLADES TAKE YOU`, overriding margin damage. Cross is attempted once per run; Leave preserves the attempt.
- The complete Room of Blades success flow passed owner Board Lab review on 2026-08-26. The owner crossed as a Rogue, equipped the Shadow Knife cache reward, observed the correct Level 3 combined totals (`HP5 POW4 DEF3 SKL6 LCK5`), and confirmed that leaving and re-entering the resolved room did not retrigger the event.
- **Strange hole in the ground:** the approved first implementation is `STRANGE HOLE`, showing `COLD AIR RISES` with `LOOK / REACH / LEAVE`. Look rolls `D6 + LUCK` versus `D6 + DANGER 3`, with ties succeeding; success reveals a transient truthful first direction on a shortest path to the hidden exit, while failure is harmless. Reach rolls `D6 + SKILL` versus `D6 + DANGER 4`, with ties failing; success opens the shared eligible cache, while failure deals 1 HP and can kill with cause `THE DARK`. Either attempted check resolves the event; an initial Leave preserves it. These danger values are playtest hypotheses rather than permanent balance commitments.
- The Strange Hole's Look success, truthful-direction clue, board presentation, and pacing passed owner Board Lab review on 2026-09-13. The Reach/cache/injury branches remain automated-test verified rather than owner-playtested.
- The private-alpha event catalog is closed at five authored rooms: Solid Door (Warrior-favored), Room of Blades and Strange Hole (Rogue-favored), Ancient Library (Wizard-favored), and class-neutral Chained Prisoner. Each run deterministically selects exactly three from topology and hidden-exit selection, without consulting the chosen class. Placement maximizes how many are outside the selected shortest entrance-to-exit route, then maximizes graph spacing; catalog tests enforce at least two graph moves between events, and the current average minimum spacing is 4.67 moves across all 30 topology/exit combinations.
- **Content-beta candidates:** Call for Help, Fresh Bread, and Loved-One Apparition remain owner-supplied seeds, not private-alpha requirements. Their mechanics are still undecided.
