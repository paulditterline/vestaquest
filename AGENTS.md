...
- Exploration uses a simple grid map that is progressively built/revealed as the character moves. The current layout idea is map on one side of the board and persistent stats on the left, but the exact split and orientation remain open.
...
- The map should read as a compact square grid made from physical color tiles. Unexplored space is blank; distinct colors communicate explored locations, dead ends, and locations where encounters occurred. The exact palette, map size, current-position treatment, and accessibility redundancies remain to be designed.
...
- Dungeon variety may come from a finite set of roughly ten authored map designs, procedural generation, or a hybrid. This remains undecided; preserve the bounded 1980s-adventure feel and guarantee that every generated run is valid and winnable.
